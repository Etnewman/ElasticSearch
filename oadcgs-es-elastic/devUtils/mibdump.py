...
#
# Purpose: This tool allows the user to compile MIBS into
#          pySNMP MIBS that can be used in the updated Elastic
#          Data Collector.
#
...
# File name: ACAS.py
#
# Location: /etc/logstash/devUtils
#
# Version: 1.0
#
...
# Deficiency: N/A
#
# Use: This is used by a user to compile necessary MIBS into pySNMP MIBS
#
...
try:
    ...
    if verboseFlag:
        ...
        sys.stderr.write(
            "Pre-compiled MIBs %sborrowed: %s\r\n"
            % (
                dryrunFlag and "Would be " or "",
                ", ".join(
                    [
                        "%s (%s)" % (x, processed[x].path)
                        for x in sorted(processed)
